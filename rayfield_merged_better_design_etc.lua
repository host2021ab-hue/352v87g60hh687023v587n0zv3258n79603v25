...
local prompt = useStudio and require(script.Parent.prompt) or (function()
	-- prompt.lua inlined (no external load needed)
	local promptRet = {}
	local _useStudio
	...
	if _runService:IsStudio() then _useStudio = true end
	local _debounce = false

	local function _open(p)
		_debounce = true
		p.Policy.Size = UDim2.new(0, 400, 0, 120)
		p.Policy.BackgroundTransparency = 1
		p.Policy.Shadow.Image.ImageTransparency = 1
		p.Policy.Title.TextTransparency = 1
		p.Policy.Notice.TextTransparency = 1
		p.Policy.Actions.Primary.BackgroundTransparency = 1
		p.Policy.Actions.Primary.Shadow.ImageTransparency = 1
		p.Policy.Actions.Primary.Title.TextTransparency = 1
		p.Policy.Actions.Secondary.Title.TextTransparency = 1
		p.Policy.Visible = true
		p.Enabled = true
		_tweenService:Create(p.Policy, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
		_tweenService:Create(p.Policy.Shadow.Image, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 0.6}):Play()
		_tweenService:Create(p.Policy, TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = UDim2.new(0, 463, 0, 150)}):Play()
		task.wait(0.15)
		_tweenService:Create(p.Policy.Title, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
		task.wait(0.03)
		_tweenService:Create(p.Policy.Notice, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.5}):Play()
		task.wait(0.15)
		_tweenService:Create(p.Policy.Actions.Primary, TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0.3}):Play()
		_tweenService:Create(p.Policy.Actions.Primary.Title, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.2}):Play()
		_tweenService:Create(p.Policy.Actions.Primary.Shadow, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 0.7}):Play()
		task.wait(5)
		if not _fin then
			_tweenService:Create(p.Policy.Actions.Secondary.Title, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.6}):Play()
			_debounce = false
		end
	end

	local function _close(p)
		_debounce = true
		_tweenService:Create(p.Policy, TweenInfo.new(0.3, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(0, 400, 0, 110)}):Play()
		_tweenService:Create(p.Policy.Title, TweenInfo.new(0.35, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Notice, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Actions.Secondary.Title, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Actions.Primary, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Actions.Primary.Title, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Actions.Primary.Shadow, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
		_tweenService:Create(p.Policy, TweenInfo.new(0.2, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
		_tweenService:Create(p.Policy.Shadow.Image, TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
		task.wait(1)
		p:Destroy()
		_fin = true
	end
	...
	function promptRet.create(title, description, primary, secondary, callback)
		local p = _useStudio and script.Parent:FindFirstChild('WarningPrompt') or game:GetObjects("rbxassetid://76963332287827")[1]
		p.Enabled = false
		if gethui then
			p.Parent = gethui()
		elseif syn and syn.protect_gui then
			syn.protect_gui(p)
			p.Parent = _coreGui
		elseif not _useStudio and _coreGui:FindFirstChild("RobloxGui") then
			p.Parent = _coreGui:FindFirstChild("RobloxGui")
		elseif not _useStudio then
			p.Parent = _coreGui
		end
		if gethui then
			for _, Interface in ipairs(gethui():GetChildren()) do
				if Interface.Name == p.Name and Interface ~= p then
					Interface.Enabled = false
					Interface.Name = "Prompt-Old"
				end
			end
		elseif not _useStudio then
			for _, Interface in ipairs(_coreGui:GetChildren()) do
				if Interface.Name == p.Name and Interface ~= p then
					Interface.Enabled = false
					Interface.Name = "Prompt-Old"
				end
		...
		p.Policy.Title.Text = title
		p.Policy.Notice.Text = description
		p.Policy.Actions.Primary.Title.Text = primary
		p.Policy.Actions.Secondary.Title.Text = secondary
		p.Policy.Actions.Primary.Interact.MouseButton1Click:Connect(function()
			_close(p)
			if callback then callback(true) end
		end)
		p.Policy.Actions.Secondary.Interact.MouseButton1Click:Connect(function()
			_close(p)
			if callback then callback(false) end
		end)
		p.Policy.Actions.Primary.Interact.MouseEnter:Connect(function()
			if _debounce then return end
			_tweenService:Create(p.Policy.Actions.Primary, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
			_tweenService:Create(p.Policy.Actions.Primary.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
			_tweenService:Create(p.Policy.Actions.Primary.Shadow, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 0.45}):Play()
		end)
		p.Policy.Actions.Primary.Interact.MouseLeave:Connect(function()
			if _debounce then return end
			_tweenService:Create(p.Policy.Actions.Primary, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2}):Play()
			_tweenService:Create(p.Policy.Actions.Primary.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.2}):Play()
			_tweenService:Create(p.Policy.Actions.Primary.Shadow, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {ImageTransparency = 0.7}):Play()
		end)
		p.Policy.Actions.Secondary.Interact.MouseEnter:Connect(function()
			if _debounce then return end
			_tweenService:Create(p.Policy.Actions.Secondary.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.3}):Play()
		end)
		p.Policy.Actions.Secondary.Interact.MouseLeave:Connect(function()
			if _debounce then return end
			_tweenService:Create(p.Policy.Actions.Secondary.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0.6}):Play()
		end)
		task.wait(0.5)
		task.spawn(_open, p)
	end
...
local RayfieldLibrary = {
	...
	Theme = {
		Default = {
			TextColor = Color3.fromRGB(235, 215, 255),

			Background = Color3.fromRGB(18, 12, 28),
			Topbar = Color3.fromRGB(28, 18, 42),
			Shadow = Color3.fromRGB(10, 6, 18),

			NotificationBackground = Color3.fromRGB(22, 14, 36),
			NotificationActionsBackground = Color3.fromRGB(200, 180, 230),

			TabBackground = Color3.fromRGB(55, 35, 80),
			TabStroke = Color3.fromRGB(75, 50, 105),
			TabBackgroundSelected = Color3.fromRGB(160, 110, 220),
			TabTextColor = Color3.fromRGB(220, 195, 255),
			SelectedTabTextColor = Color3.fromRGB(18, 10, 30),

			ElementBackground = Color3.fromRGB(30, 18, 48),
			ElementBackgroundHover = Color3.fromRGB(42, 26, 64),
			SecondaryElementBackground = Color3.fromRGB(22, 14, 36),
			ElementStroke = Color3.fromRGB(70, 45, 105),
			SecondaryElementStroke = Color3.fromRGB(55, 35, 82),

			SliderBackground = Color3.fromRGB(120, 60, 200),
			SliderProgress = Color3.fromRGB(150, 80, 230),
			SliderStroke = Color3.fromRGB(175, 110, 255),

			ToggleBackground = Color3.fromRGB(28, 16, 44),
			ToggleEnabled = Color3.fromRGB(140, 70, 220),
			ToggleDisabled = Color3.fromRGB(75, 55, 100),
			ToggleEnabledStroke = Color3.fromRGB(170, 100, 255),
			ToggleDisabledStroke = Color3.fromRGB(100, 75, 130),
			ToggleEnabledOuterStroke = Color3.fromRGB(90, 55, 140),
			ToggleDisabledOuterStroke = Color3.fromRGB(50, 35, 75),

			DropdownSelected = Color3.fromRGB(45, 28, 70),
			DropdownUnselected = Color3.fromRGB(28, 18, 46),

			InputBackground = Color3.fromRGB(28, 18, 44),
			InputStroke = Color3.fromRGB(80, 55, 115),
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	-- Theme immer anwenden (Default = Purple, oder das angegebene Theme)
	if Settings.Theme then
		...
		-- Kein Theme angegeben → Default (Lila) erzwingen
		pcall(ChangeTheme, 'Default')
	end

	Topbar.Visible = false
	Elements.Visible = false
	LoadingFrame.Visible = false  -- CigaretteLoader übernimmt
	Main.Visible = false          -- Rayfield komplett versteckt bis Loader fertig

	-- ══════════════════════════════════════════════════════
	--  CIGARETTE LOADER — läuft parallel, signalisiert via _cigDone
	-- ══════════════════════════════════════════════════════
	local _cigDone = false

	task.spawn(function()
		local _CigTween2      = game:GetService("TweenService")
		local _CigLocalPlayer2 = game:GetService("Players").LocalPlayer
		local _CigPlayerGui2  = _CigLocalPlayer2:WaitForChild("PlayerGui")
		local _cigIsDestroyed = false
		local _cigProgress    = 0

		local function _cigLerp(a, b, t) return a + (b - a) * t end
		local function _cigRandF(a, b)   return a + math.random() * (b - a) end

		local function _cigMakeTween(obj, t, style, dir, props)
			return _CigTween2:Create(obj,
				TweenInfo.new(t, style or Enum.EasingStyle.Quad, dir or Enum.EasingDirection.Out), props)
		end

		local function _cigFrame(parent, props)
		...
		local function _cigCorner(parent, rad)
			local c = Instance.new("UICorner")
			c.CornerRadius = rad or UDim.new(0,4)
			c.Parent = parent
			return c
		end

		local function _cigGradient(parent, seq, rotation)
		...
		-- ScreenGui
		local _CigGui = Instance.new("ScreenGui")
		_CigGui.Name           = "CigaretteLoader"
		_CigGui.ResetOnSpawn   = false
		_CigGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
		_CigGui.IgnoreGuiInset = true
		_CigGui.DisplayOrder   = 999
		_CigGui.Parent         = _CigPlayerGui2

		local _CigBg = _cigFrame(_CigGui, {
			Name = "Bg", Size = UDim2.new(1,0,1,0),
			BackgroundColor3 = Color3.fromRGB(8,6,10), ZIndex = 1,
		})

		local _CigVig = _cigFrame(_CigGui, {
			Size = UDim2.new(1,0,1,0),
			BackgroundColor3 = Color3.fromRGB(0,0,0),
			BackgroundTransparency = 0.55, ZIndex = 2,
		})
		_cigGradient(_CigVig, ColorSequence.new({
			ColorSequenceKeypoint.new(0,   Color3.fromRGB(0,0,0)),
			ColorSequenceKeypoint.new(0.45,Color3.fromRGB(255,255,255)),
			ColorSequenceKeypoint.new(1,   Color3.fromRGB(0,0,0)),
		}), 90)

		local _CigCont = _cigFrame(_CigGui, {
			Size = UDim2.new(0, 420, 0, 270),
			Position = UDim2.new(0.5, -210, 0.5, -135),
			BackgroundColor3 = Color3.fromRGB(14, 11, 18), ZIndex = 3,
		})
		_cigCorner(_CigCont, UDim.new(0,20))

		local _ContStroke = Instance.new("UIStroke")
		_ContStroke.Color       = Color3.fromRGB(90, 65, 110)
		_ContStroke.Thickness   = 1.5
		_ContStroke.Transparency = 0.4
		_ContStroke.Parent      = _CigCont

		local _CigInnerGlow = _cigFrame(_CigCont, {
			Size = UDim2.new(1,0,0,90), Position = UDim2.new(0,0,0,0),
			BackgroundColor3 = Color3.fromRGB(255,160,60),
			BackgroundTransparency = 0.96, ZIndex = 4,
		})
		_cigCorner(_CigInnerGlow, UDim.new(0,20))

		-- Titel
		local _CigTitle = Instance.new("TextLabel")
		_CigTitle.Size = UDim2.new(1,-40,0,32)
		_CigTitle.Position = UDim2.new(0,20,0,18)
		_CigTitle.BackgroundTransparency = 1
		_CigTitle.Text = "LOADING"
		_CigTitle.TextColor3 = Color3.fromRGB(235, 215, 255)
		_CigTitle.TextSize = 20
		_CigTitle.Font = Enum.Font.GothamBold
		_CigTitle.TextXAlignment = Enum.TextXAlignment.Center
		_CigTitle.ZIndex = 5
		_CigTitle.Parent = _CigCont

		_cigFrame(_CigCont, {
			Size = UDim2.new(0,260,0,1), Position = UDim2.new(0.5,-130,0,56),
			BackgroundColor3 = Color3.fromRGB(80,55,105),
			BackgroundTransparency = 0.5, ZIndex = 4,
		})
		...
		local CIG_H    = 30
		local FILTER_W = 58
		local BODY_W   = CIG_W - FILTER_W

		local _CigCigCont = _cigFrame(_CigCont, {
			Size = UDim2.new(0, CIG_W, 0, CIG_H + 20),
			Position = UDim2.new(0.5, -CIG_W/2, 0, 75),
			BackgroundTransparency = 1, ZIndex = 4,
		})

		-- Filter
		local _CigFilter = _cigFrame(_CigCigCont, {
			Size = UDim2.new(0, FILTER_W, 0, CIG_H),
			Position = UDim2.new(0, BODY_W, 0, 10),
			BackgroundColor3 = Color3.fromRGB(190, 125, 68), ZIndex = 5,
		})
		_cigCorner(_CigFilter, UDim.new(0,3))
		_cigGradient(_CigFilter, ColorSequence.new({
			ColorSequenceKeypoint.new(0,   Color3.fromRGB(215, 155, 95)),
			ColorSequenceKeypoint.new(0.5, Color3.fromRGB(178, 112, 58)),
			ColorSequenceKeypoint.new(1,   Color3.fromRGB(138,  82, 38)),
		}), 90)
		for i = 1, 5 do
			_cigFrame(_CigFilter, {
				Size = UDim2.new(0, 1, 0, CIG_H - 6),
				Position = UDim2.new(0, 4 + (i-1)*9, 0, 3),
				BackgroundColor3 = Color3.fromRGB(225, 170, 108),
				BackgroundTransparency = 0.45, ZIndex = 6,
			})
		end

		_cigFrame(_CigCigCont, {
			Size = UDim2.new(0, 2, 0, CIG_H),
			Position = UDim2.new(0, BODY_W - 1, 0, 10),
			BackgroundColor3 = Color3.fromRGB(55,40,25), ZIndex = 7,
		})

		-- Körper
		local _CigBody = _cigFrame(_CigCigCont, {
			Size = UDim2.new(0, BODY_W, 0, CIG_H),
			Position = UDim2.new(0, 0, 0, 10),
			BackgroundColor3 = Color3.fromRGB(238, 233, 228),
			ZIndex = 5, ClipsDescendants = false,
		})
		_cigCorner(_CigBody, UDim.new(0,3))
		_cigGradient(_CigBody, ColorSequence.new({
			ColorSequenceKeypoint.new(0,   Color3.fromRGB(198, 193, 186)),
			ColorSequenceKeypoint.new(0.2, Color3.fromRGB(244, 240, 236)),
			ColorSequenceKeypoint.new(0.8, Color3.fromRGB(238, 233, 228)),
			ColorSequenceKeypoint.new(1,   Color3.fromRGB(188, 181, 173)),
		}), 90)
		_cigFrame(_CigBody, {
			Size = UDim2.new(1,0,0,1), Position = UDim2.new(0,0,0.5,-0.5),
			BackgroundColor3 = Color3.fromRGB(178,170,160),
			BackgroundTransparency = 0.65, ZIndex = 6,
		})

		-- Asche
		local _CigAsh = _cigFrame(_CigBody, {
			Size = UDim2.new(0, 0, 1, 0), Position = UDim2.new(0,0,0,0),
			BackgroundColor3 = Color3.fromRGB(45,42,50),
			ZIndex = 7, ClipsDescendants = true,
		})
		_cigCorner(_CigAsh, UDim.new(0,3))
		_cigGradient(_CigAsh, ColorSequence.new({
			ColorSequenceKeypoint.new(0,   Color3.fromRGB(32,30,38)),
			ColorSequenceKeypoint.new(0.45,Color3.fromRGB(60,56,68)),
			ColorSequenceKeypoint.new(1,   Color3.fromRGB(40,38,46)),
		}), 90)
		_cigFrame(_CigAsh, {
			Size = UDim2.new(1,0,0,9), Position = UDim2.new(0,0,0,2),
			BackgroundColor3 = Color3.fromRGB(145,138,150),
			BackgroundTransparency = 0.5, ZIndex = 8,
		})
		for i = 1, 10 do
			_cigFrame(_CigAsh, {
				Size = UDim2.new(0, math.random(2,6), 0, math.random(1,3)),
				Position = UDim2.new(math.random(4,90)/100, 0, math.random(15,78)/100, 0),
				BackgroundColor3 = Color3.fromRGB(math.random(95,160), math.random(90,155), math.random(100,165)),
				BackgroundTransparency = math.random(30,65)/100, ZIndex = 9,
			})
		end

		-- Glut
		local _CigEmberGlow = _cigFrame(_CigBody, {
			Size = UDim2.new(0, 22, 1, 12), Position = UDim2.new(0, -6, 0, -6),
			BackgroundColor3 = Color3.fromRGB(255, 120, 20),
			BackgroundTransparency = 0.65, ZIndex = 8,
		})
		_cigCorner(_CigEmberGlow, UDim.new(1,0))

		local _CigEmberMid = _cigFrame(_CigBody, {
			Size = UDim2.new(0, 12, 1, 4), Position = UDim2.new(0, 0, 0, 0),
			BackgroundColor3 = Color3.fromRGB(245, 90, 10), ZIndex = 9,
		})
		_cigCorner(_CigEmberMid, UDim.new(0,4))

		local _CigEmberCore = _cigFrame(_CigBody, {
			Size = UDim2.new(0, 7, 0, CIG_H - 12), Position = UDim2.new(0, 2, 0, 6),
			BackgroundColor3 = Color3.fromRGB(255, 230, 180), ZIndex = 10,
		})
		_cigCorner(_CigEmberCore, UDim.new(0,3))

		-- Labels
		local _CigPct = Instance.new("TextLabel")
		_CigPct.Size = UDim2.new(0, 80, 0, 24)
		_CigPct.Position = UDim2.new(0.5, -40, 0, 120)
		_CigPct.BackgroundTransparency = 1
		_CigPct.Text = "0%"
		_CigPct.TextColor3 = Color3.fromRGB(200, 180, 230)
		_CigPct.TextSize = 18
		_CigPct.Font = Enum.Font.GothamBold
		_CigPct.ZIndex = 5
		_CigPct.Parent = _CigCont

		local _CigStatus = Instance.new("TextLabel")
		_CigStatus.Size = UDim2.new(1, -40, 0, 20)
		_CigStatus.Position = UDim2.new(0, 20, 0, 148)
		_CigStatus.BackgroundTransparency = 1
		_CigStatus.Text = "Initialisiere..."
		_CigStatus.TextColor3 = Color3.fromRGB(140, 110, 180)
		_CigStatus.TextSize = 13
		_CigStatus.Font = Enum.Font.Gotham
		_CigStatus.TextXAlignment = Enum.TextXAlignment.Center
		_CigStatus.ZIndex = 5
		_CigStatus.Parent = _CigCont

		-- Rauch
		local function _cigSpawnSmoke()
			if _cigIsDestroyed then return end
			local ashWidth = math.floor(BODY_W * _cigProgress)
			if ashWidth < 3 then return end
			local bPos = _CigBody.AbsolutePosition
			local sx = bPos.X + ashWidth - 4
			local sy = bPos.Y + CIG_H/2
			local s = _cigFrame(_CigGui, {
				Size = UDim2.new(0, math.random(6,14), 0, math.random(6,14)),
				Position = UDim2.new(0, sx - 5, 0, sy - 10),
				BackgroundColor3 = Color3.fromRGB(math.random(180,220), math.random(170,210), math.random(185,225)),
				BackgroundTransparency = 0.5, ZIndex = 20,
			})
			_cigCorner(s, UDim.new(1,0))
			local drift = math.random(-18, 18)
			local rise  = math.random(28, 55)
			local dur   = _cigRandF(0.9, 1.8)
			_cigMakeTween(s, dur, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, {
				Position = UDim2.new(0, sx - 5 + drift, 0, sy - 10 - rise),
				Size = UDim2.new(0, math.random(20,38), 0, math.random(20,38)),
		...
		-- Glut-Position
		local function _cigSetEmberX(ashWidth)
			local ex = math.max(0, ashWidth - 5)
			_CigEmberGlow.Position = UDim2.new(0, ex - 6, 0, -5)
			_CigEmberMid.Position  = UDim2.new(0, ex, 0, 0)
			_CigEmberCore.Position = UDim2.new(0, ex + 2, 0, 6)
		end

		-- Fortschritt setzen
		local function _cigSetProgress(value, statusText)
			if _cigIsDestroyed then return end
			_cigProgress = math.clamp(value, 0, 1)
			local ashWidth = math.floor(BODY_W * _cigProgress)
			_cigMakeTween(_CigAsh, 0.55, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, {
				Size = UDim2.new(0, ashWidth, 1, 0)
			}):Play()
			task.delay(0.08, function()
				if not _cigIsDestroyed then _cigSetEmberX(ashWidth) end
			end)
			local targetPct  = math.floor(_cigProgress * 100)
			local currentPct = tonumber(_CigPct.Text:match("%d+")) or 0
			task.spawn(function()
				for p = currentPct, targetPct do
					if _cigIsDestroyed then break end
					_CigPct.Text = p .. "%"
					task.wait(0.012)
				end
			end)
			if statusText and statusText ~= _CigStatus.Text then
				_cigMakeTween(_CigStatus, 0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
					{TextTransparency = 1}):Play()
				task.delay(0.17, function()
					if not _cigIsDestroyed then
						_CigStatus.Text = statusText
						_cigMakeTween(_CigStatus, 0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
		...
		-- Animations-Loops
		task.spawn(function()
			while not _cigIsDestroyed do
				local t = _cigRandF(0.25, 0.65)
				if _CigEmberGlow and _CigEmberGlow.Parent then
					_cigMakeTween(_CigEmberGlow, t, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
						{BackgroundTransparency = _cigRandF(0.58, 0.80)}):Play()
				end
				if _CigEmberMid and _CigEmberMid.Parent then
					_cigMakeTween(_CigEmberMid, t, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, {
						BackgroundColor3 = Color3.fromRGB(math.random(228,255), math.random(68,112), math.random(5,22)),
					}):Play()
				end
				if _CigEmberCore and _CigEmberCore.Parent then
					_cigMakeTween(_CigEmberCore, t, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
						{BackgroundTransparency = _cigRandF(0.0, 0.28)}):Play()
				end
				task.wait(t)
			end
		end)
		...
		task.spawn(function()
			while not _cigIsDestroyed do
				local t = _cigRandF(1.4, 3.0)
				if _CigInnerGlow and _CigInnerGlow.Parent then
					_cigMakeTween(_CigInnerGlow, t, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut,
						{BackgroundTransparency = _cigRandF(0.91, 0.97)}):Play()
				end
				task.wait(t)
			end
		end)
		...
		task.spawn(function()
			task.wait(0.6)
			while not _cigIsDestroyed do
				if _cigProgress > 0.01 then
					local count = math.max(1, math.floor(1 + _cigProgress * 2.5))
					for _ = 1, count do
						task.spawn(_cigSpawnSmoke)
						task.wait(0.06)
					end
				end
				task.wait(_cigLerp(0.65, 0.16, _cigProgress))
			end
		end)

		-- Intro-Animation
		_CigBg.BackgroundTransparency   = 1
		_CigCont.BackgroundTransparency = 1
		_CigCont.Position = UDim2.new(0.5, -210, 0.56, -135)
		for _, obj in ipairs(_CigCont:GetDescendants()) do
			if obj:IsA("TextLabel") then obj.TextTransparency = 1 end
		end
		_cigMakeTween(_CigBg, 0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
			{BackgroundTransparency = 0}):Play()
		task.wait(0.25)
		_cigMakeTween(_CigCont, 0.65, Enum.EasingStyle.Back, Enum.EasingDirection.Out, {
		...
		for _, obj in ipairs(_CigCont:GetDescendants()) do
			if obj:IsA("TextLabel") then
				_cigMakeTween(obj, 0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
					{TextTransparency = 0}):Play()
			end
		end
		...
		-- Lade-Simulation
		local _statusMsgs = {
			{ at = 0.05, msg = "Verbinde mit Server..."  },
			{ at = 0.18, msg = "Authentifiziere..."       },
			{ at = 0.32, msg = "Lade Spielerdaten..."     },
			{ at = 0.46, msg = "Initialisiere Module..."  },
			{ at = 0.58, msg = "Überprüfe Assets..."      },
			{ at = 0.70, msg = "Patche Funktionen..."     },
			{ at = 0.82, msg = "Injiziere Scripts..."     },
			{ at = 0.93, msg = "Letzte Züge..."           },
			{ at = 1.00, msg = "Fertig."                  },
		}
		local steps = 80
		for i = 1, steps do
			if _cigIsDestroyed then break end
			task.wait(_cigRandF(0.04, 0.11))
			local p = i / steps
			local msg = nil
			for _, entry in ipairs(_statusMsgs) do
				if math.abs(p - entry.at) < (1.5/steps) then msg = entry.msg break end
			end
			_cigSetProgress(p, msg)
		end

		-- Outro
		task.wait(0.3)
		if not _cigIsDestroyed then
			_cigMakeTween(_CigTitle, 0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
				{TextColor3 = Color3.fromRGB(140, 240, 160)}):Play()
			_CigTitle.Text = "✓  BEREIT"
			_cigMakeTween(_CigEmberGlow, 0.9, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
				{BackgroundTransparency = 1}):Play()
			_cigMakeTween(_CigEmberMid, 0.9, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
				{BackgroundColor3 = Color3.fromRGB(48,44,54)}):Play()
			_cigMakeTween(_CigEmberCore, 0.9, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
				{BackgroundTransparency = 1}):Play()
			task.wait(1.1)
		end
		if not _cigIsDestroyed then
			_cigMakeTween(_CigCont, 0.65, Enum.EasingStyle.Quad, Enum.EasingDirection.In, {
				BackgroundTransparency = 1,
				Position = UDim2.new(0.5,-210,0.58,-135),
			}):Play()
			for _, obj in ipairs(_CigCont:GetDescendants()) do
				if obj:IsA("TextLabel") then
					_cigMakeTween(obj, 0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
						{TextTransparency = 1}):Play()
				end
			...
			_cigMakeTween(_CigBg, 0.85, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
				{BackgroundTransparency = 1}):Play()
			task.wait(0.9)
			_cigIsDestroyed = true
			_CigGui:Destroy()
		end

		_cigDone = true  -- Signal: Rayfield darf sich zeigen
	end)
	-- ══════════════════════════════════════════════════════
	...
	-- CigaretteLoader bereits aktiv (gestartet vor KeySystem)
	-- Warte bis der Loader fertig ist
	repeat task.wait(0.05) until _cigDone
	...
	-- Rayfield UI einblenden (CigaretteLoader bereits fertig, da wir gewartet haben)
	Main.Visible = true
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
